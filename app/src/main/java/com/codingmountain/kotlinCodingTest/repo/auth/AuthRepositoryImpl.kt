...
constructor(
    ...
    override fun signUp(email: String, password: String, status: (Resource<AuthResult>) -> Unit) {
    ...
    override fun signIn(email: String, password: String, status: (Resource<AuthResult>) -> Unit) {
        status(Resource.Loading)
        try {
            if (emailValid(email, status) && passwordValid(password, status)) {
